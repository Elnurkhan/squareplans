...
export class TextureLoader {
  ...
  async load({ ktx2, fallback, ...textureOpts }) {
    ...
    try {
      ...
      console.warn(`[TextureLoader] failed to load ${src}:`, err)
      // If KTX2 path failed and we have a fallback, retry with image
      if (src === ktx2 && fallback) {
        try {
          await this._loadImage(texture, fallback)
          this._cache.set(fallback, texture)
          return texture
        } catch (err2) {
          console.warn(`[TextureLoader] fallback also failed ${fallback}:`, err2)
          throw err2
        }
      }
      throw err
  ...
  async _loadKTX2(texture, url) {
    ...
    if (!response.ok) {
      throw new Error(`KTX2 fetch failed: ${response.status} ${response.statusText}`)
    }
